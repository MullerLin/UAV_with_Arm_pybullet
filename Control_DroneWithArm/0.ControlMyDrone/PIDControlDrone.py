...
orientation = np.pi/6
period = int(NUM_WP/(duration_sec/To_sec))
Target_VEC = [orientation, orientation, orientation]
for i in range (period):
    Trajectory_ORN[i, :] = np.array(INIT_RPYS)
for i in range (period, 2*period):
    Trajectory_ORN[i, :] = np.array(Target_VEC) / NUM_TO * (i+1-NUM_TO) + np.array(INIT_RPYS)
for i in range (2*period, 3*period):
    Trajectory_ORN[i, :] = np.array(Target_VEC) + np.array(INIT_RPYS)
orn_drone = np.array(Target_VEC) + np.array(INIT_RPYS)
Target_VEC  = [-orientation, -orientation, -orientation]
for i in range (3*period, 4*period):
    Trajectory_ORN[i, :] = np.array(Target_VEC) / NUM_TO * (i+1-3*NUM_TO) + orn_drone
for i in range (4*period, 5*period):
    Trajectory_ORN[i, :] = np.array(Target_VEC) + orn_drone
