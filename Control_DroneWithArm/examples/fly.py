...
if __name__ == "__main__":
    ...
    INIT_XYZS = np.array([[R*np.cos((i/6)*2*np.pi+np.pi/2), R*np.sin((i/6)*2*np.pi+np.pi/2)-R, 1] for i in range(ARGS.num_drones)])
    ...
    elif ARGS.drone in [DroneModel.HB, DroneModel.HBARM]:
